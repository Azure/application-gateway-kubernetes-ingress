...
--}`
	...
	Context("Tests Application Gateway config creation with appgw-ssl-certificate annotation", func() {
		ingressWithoutTLSSpec := &v1beta1.Ingress{
			Spec: v1beta1.IngressSpec{
				Rules: []v1beta1.IngressRule{
					{
						Host: "foo.baz",
		...
		cbCtx := &ConfigBuilderContext{
			IngressList:           []*v1beta1.Ingress{ingressWithoutTLSSpec},
			ServiceList:           serviceList,
			EnvVariables:          environment.GetFakeEnv(),
			DefaultAddressPoolID:  to.StringPtr("xx"),
			DefaultHTTPSettingsID: to.StringPtr("yy"),
...
--                    "protocol": "Https",
--                    "sslCertificate": {
--                        "id": "/subscriptions/--subscription--/resourceGroups/--resource-group--/providers/Microsoft.Network/applicationGateways/--app-gw-name--/sslCertificates/appgw-installed-cert"
--                    }
--                }
--            }
--        ],
