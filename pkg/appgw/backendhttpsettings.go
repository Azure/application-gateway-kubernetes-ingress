...
func (c *appGwConfigBuilder) resolveBackendPort(backendID backendIdentifier) (Port, error) {
	...
	if backendID.isLDPBackend() {
		backendLdp, _ := c.k8sContext.GetLoadDistributionPolicy(backendID.Namespace, backendID.Backend.Resource.Name)
		service = c.k8sContext.GetService(fmt.Sprintf("%v/%v", backendID.Namespace, backendLdp.Spec.Targets[0].Backend.Service.Name))
		backendService = backendLdp.Spec.Targets[0].Backend.Service
...
func (c *appGwConfigBuilder) generateHTTPSettings(backendID backendIdentifier, port Port, cbCtx *ConfigBuilderContext) n.ApplicationGatewayBackendHTTPSettings {
	var backendService = &networking.IngressServiceBackend{}
	if backendID.isLDPBackend() {
	...
	httpSettingsName := generateHTTPSettingsName(backendID.serviceFullName(), serviceBackendPortToStr(backendService.Port), port, backendID.Ingress.Name)
