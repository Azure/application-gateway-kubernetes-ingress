...
var _ = Describe("Test the creation of Backend http settings from Ingress definition", func() {
	...
	_ = configBuilder.k8sContext.Caches.LoadDistributionPolicy.Add(loadDistributionPolicy)
	...
	Context("test backend port resolution for load distribution policy backend that doesnt exist", func() {
		...
		_ = configBuilder.k8sContext.Caches.LoadDistributionPolicy.Delete(loadDistributionPolicy)
